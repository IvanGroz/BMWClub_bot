...
def register_user_plus_handlers(dp: Dispatcher) -> None:
    ...
    dp.register_message_handler(off_birthday_notif, IsPlusUser(), content_types=['text'],
                                text='Отключить уведомления о ДР')
    dp.register_message_handler(on_birthday_notif, IsPlusUser(), content_types=['text'],
                                text='Включить уведомления о ДР')
    dp.register_message_handler(get_menu, IsRegularUserOnly(), commands=['main_menu'])

    dp.register_message_handler(birthday_menu, IsPlusUser(), text='Дни рождения')
